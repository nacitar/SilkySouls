...
{
    public class TravelServiceOld(IMemoryService memoryService, HookManager hookManager)
    {
        public void Warp(WarpLocation selectedWarpLocation)
        {
            ...
            byte[] bytes = BitConverter.GetBytes(EventMan.Base);
            Array.Copy(bytes, 0, warpBytes, 2, 8);
            bytes = BitConverter.GetBytes(Functions.Warp);
